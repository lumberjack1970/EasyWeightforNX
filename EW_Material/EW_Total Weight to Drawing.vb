...
Module NXJournal
    ...
    Sub FindBody(myComp As NXOpen.Assemblies.Component, ByRef totalAssemblyWeight As Double, ByRef totalRAW_DT_AssemblyWeight As Double, lw As NXOpen.ListingWindow, ByRef numberOfSolidBodies As Integer)
        ...
        For Each myBody As NXOpen.Body In workPart.Bodies
            ' lw.WriteLine("Processing body: " & myBody.JournalIdentifier)

            ' Handle EW_Body_Weight
            Try
                Dim bodyWeightStr = myBody.GetStringAttribute("EW_Body_Weight")
                If Not String.IsNullOrEmpty(bodyWeightStr) Then
                    Dim bodyWeight As Double = 0
                    If Double.TryParse(bodyWeightStr, bodyWeight) Then
                        totalAssemblyWeight += bodyWeight
                        numberOfSolidBodies += 1
                    Else
                        lw.WriteLine("Failed to decipher the weight value for this body: " & bodyWeightStr)
                    End If
                Else
                    'lw.WriteLine("In a most peculiar twist, the EW_Body_Weight attribute is null or,")
                    'lw.WriteLine("one might say, as empty as deep space.")
                End If
            Catch ex As Exception
                'lw.WriteLine("An error occurred while processing EW_Body_Weight attribute: " & ex.Message)
            End Try

            ' Handle Raw_Body_Delta_Weight
            Try
                Dim RawWeightStr = myBody.GetStringAttribute("Raw_Body_Delta_Weight")
                If Not String.IsNullOrEmpty(RawWeightStr) Then
                    ...
                    End If
                Else
                    'lw.WriteLine("In a most peculiar twist, the Raw_Body_Delta_Weight attribute is null or,")
                    'lw.WriteLine("one might say, as empty as deep space.")
                End If
            Catch ex As Exception
                'lw.WriteLine("An error occurred while processing Raw_Body_Delta_Weight attribute: " & ex.Message)
            End Try
		Next
	End Sub
End Module